  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="p-4">
          <div className="text-xs uppercase tracking-wide text-white/40">Soonest deadline</div>
          {soonest ? (
            <Link href={`/incidents/${soonest.id}`} className="block mt-1">
              <Countdown deadline={soonest.next_deadline} />
              <div className="text-xs text-white/50 font-mono">{soonest.sku} · {soonest.cve_id}</div>
            </Link>
          ) : (
            <div className="mt-1 text-white/40">—</div>
          )}
        </Card>
        <Card className="p-4 text-sm space-y-1">
          <div className="text-xs uppercase tracking-wide text-white/40">System</div>
          ...
          <div className="flex justify-between"><span>Local model</span>{modelReady ? <Badge value="ok" /> : <span className="text-xs text-amber-300">pulling…</span>}</div>
          <div className="text-[11px] text-white/40">{health?.ollama.models?.join(", ") || health?.ollama.target}</div>
          <div className="text-[11px] text-white/40">KEV sync: {kev ? formatDateTime(kev.finished_at) : "never"}</div>
        </Card>
      </section>
...
function Step({ n, title, body }: { n: string; title: string; body: string }) {
  return (
    <Card className="p-4">
      <div className="text-xs text-amber-300 font-mono">step {n}</div>
      <div className="font-medium mt-1">{title}</div>
      <p className="text-white/50 mt-1">{body}</p>
    </Card>
  );
}
